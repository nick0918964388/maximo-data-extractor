...
class PostgreSQLTransfer:
    ...
    def get_last_date(self, table_name: str, date_column: str) -> str | None:
        """Query the MAX value of a date column from PostgreSQL table.
        Returns the date string or None if table doesn't exist or is empty.
        """
        try:
            conn = psycopg2.connect(
                host=self.host, port=self.port, dbname=self.database,
            ...
            if not cur.fetchone()[0]:
                ...
                return None
            # Get MAX date value
            cur.execute(f'SELECT MAX("{date_column}") FROM "{table_name}" WHERE "{date_column}" IS NOT NULL AND "{date_column}" != \'\'')
            result = cur.fetchone()
            cur.close()
            conn.close()
