...
export const testPgConnection = (data) => api.post('/connection/test-pg', data).then(r => r.data)
...
export const cancelExtract = (profile_id) => api.post(`/extract/cancel/${profile_id}`).then(r => r.data)

// History
export const listHistory = (limit = 50, tenant_id) => api.get('/history', { params: { limit, tenant_id } }).then(r => r.data)
...
export const previewDbTable = (table_name, page = 1, page_size = 100, sort_by = null, sort_order = 'asc', filters = null) =>
  api.get(`/preview/db/${table_name}`, { params: { page, page_size, sort_by, sort_order, filters: filters ? JSON.stringify(filters) : null } }).then(r => r.data)
